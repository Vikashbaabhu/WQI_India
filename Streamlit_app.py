...
def wqix(year,temp,do,ph,con,bod,nit,fec,tot):
     
    prediction=classifier.predict([[year,temp,do,ph,con,bod,nit,fec,tot]])
    print(prediction)
    return prediction 
def main():
    html_temp = """
    <div style="background-color:skyblue;padding:10px;border-radius:5px">
    <h2 style="color:darkblue;text-align:center;font-family:Callibri;"><b>Indian River Water Quality Prediction</b></h2>
    </div>
    <br></br>
    """
   ...
   #   st.markdown(f'<style>{f.read()}</style>', unsafe_allow_html=True)    
    ...
    if st.button("**Predict**"):
        ...
        c3.text_area(label="**Conductivity**", value="{:.2f}".format(278))
        c1.write("")
        c1.write("")
        c1.write("")
        c1.write("")
        ...
        c2.text_area(label="**Fecal Coliform**", value="{:.2f}".format(fec[0]*6))
        c3.text_area(label="**Total Coliform**", value="{:.2f}".format(tot[0]*5))
